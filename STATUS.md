...
> Обновлено: 2026-03-16 (реализованы все пункты без внешних зависимостей)
...
|----------------------------|-------------------------------------------------------------------------------------------------|
| Реальная логика ИИ-анализа | `repo.start_analysis()` / `repo.get_latest_analysis()` — заглушки; реальная LLM не подключена   |
| Core API — MVP-заглушки    | Уведомления/workflow через БД или WebSocket не реализованы, только логирование                  |
...
| Проблема                     | Комментарий                                                                  |
|------------------------------|------------------------------------------------------------------------------|
| Реальная LLM-интеграция      | `ai_plan.py`, `ai_sources.py` — MVP-заглушки (возвращают шаблонные данные)   |
| Парсер внешних источников    | `POST /sources/fetch` возвращает пустой `{}` — скрейпер не реализован        |
| Реальная публикация TG/VK    | `PublishingService` — вероятно заглушка (нет реальных API-вызовов)           |
...
|-----------------|------------------|----------------------------------------|--------------------|---------------------|-------------|
| **Legal**       | ✅ Полная        | ⚠️ API есть, LLM заглушка             | ✅ Outbox          | ✅ Реализован       | ❌ Пусто    |
| **Marketing**   | ✅ Полная        | ⚠️ API есть, LLM + публикация заглушки| ✅ N1–N4           | ✅ Проверки есть   | ❌ Заготовка|
